...
namespace UII
{
	...
	namespace Sidebar
	{
		...
		void startSidebar()
		{
			...
			ImGui::BeginChild("Sidebar", ImVec2(barSize.x, sizeAvail.y - 100), false);
			pos = ImGui::GetCursorScreenPos();
			sizeAvail = ImGui::GetContentRegionAvail();
	...
	namespace Controls
	{
		...
		void SliderInt(std::string name, int* out, int min, int max)
		{
			bool hovering = false;
			ImDrawList* drawList = ImGui::GetWindowDrawList();
			ImVec2 pos = ImGui::GetCursorScreenPos();
			...
			pos = ImGui::GetCursorScreenPos();
			ImGui::PushStyleVar(ImGuiStyleVar_Alpha, 0.f); ///Im mad lazy
			ImGui::SliderInt(name.c_str(), out, min, max);
			ImGui::PopStyleVar();
			ImVec2 size = ImGui::GetItemRectSize();
...
void GUI::Begin()
{
	...
	ImGui::Begin(xorstr_("Sentry Cheats"), nullptr, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoBackground);
}
