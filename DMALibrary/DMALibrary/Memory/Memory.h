...
public:
	...
	*/
	...
	/**
	 * \brief  Gets the DTBS of our system
	 * \return a vector with all DTBS on our system.
	 */
	...
	bool ValidPtr(uintptr_t addr, bool kernel = false) const;
	...
	*/
	...
	bool Read(uintptr_t address, void* buffer, size_t size, int pid, bool kernel = false) const;
	...
	*/
	template <typename T>
	T Read(void* address, int pid, bool kernel = false)
	{
		T buffer { };
		memset(&buffer, 0, sizeof(T));
		Read(reinterpret_cast<uint64_t>(address), reinterpret_cast<void*>(&buffer), sizeof(T), pid, kernel);

		return buffer;
	}

	template <typename T>
	T Read(uint64_t address, int pid, int kernel = false)
	{
		return Read<T>(reinterpret_cast<void*>(address), pid, kernel);
	}
