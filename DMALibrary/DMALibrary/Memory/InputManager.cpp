...
bool c_keys::InitKeyboard()
{
	...
	if (Winver > 22000)
	{
		...
		if (!VMMDLL_ProcessGetInformationAll(mem.vHandle, &process_info, &total_processes))
		{
		...
		for (size_t i = 0; i < total_processes; i++)
		{
			...
			auto pid = mem.GetPidFromName(process.szNameLong);
			...
			if (tmp)
			{
				...
				printf("g_session_global_slots: 0x%p\n", g_session_global_slots);
				...
				printf("user_session_state: 0x%p\n", user_session_state);
		...
		//Second method to get it...

		int pid = mem.GetPidFromName("csrss.exe");
		PVMMDLL_MAP_MODULEENTRY module_info;
		auto result = VMMDLL_Map_GetModuleFromNameW(mem.vHandle, pid | VMMDLL_PID_PROCESS_WITH_KERNELMEMORY, static_cast<LPCWSTR>(L"win32ksgd.sys"), &module_info, VMMDLL_MODULE_FLAG_NORMAL);
		if (!result)
		{
		...
		char str[32];
		if (!VMMDLL_PdbLoad(mem.vHandle, pid | VMMDLL_PID_PROCESS_WITH_KERNELMEMORY, module_info->vaBase, str))
		{
		...
		if (!VMMDLL_PdbSymbolAddress(mem.vHandle, str, const_cast<LPSTR>("gSessionGlobalSlots"), &g_session_global_slots))
		{
			LOG("failed to find gSessionGlobalSlots\n");
			return false;
		}

		printf("First Read: %llx\n", mem.Read<uintptr_t>(g_session_global_slots, pid));
		printf("Second Read: %llx\n", mem.Read<uintptr_t>(mem.Read<uintptr_t>(g_session_global_slots, pid), pid));
